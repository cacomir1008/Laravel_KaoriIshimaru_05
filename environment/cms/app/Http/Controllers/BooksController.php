...
{
    //コンストラクタ （このクラスが呼ばれたら最初に処理をする）※ここの位置で良いのかな？
    ...
    public function index2(){
        $company = Book::where('item_category','仕事の悩み')
        ->orderBy('created_at', 'asc')
        ->paginate(3);
        return view('books', [
            'books' => $company
    ...
    public function index3(){
        $company = Book::where('item_category','家庭の悩み')
        ->orderBy('created_at', 'asc')
        ->paginate(3);
        return view('books', [
            'books' => $company
    ...
    public function index4(){
        $company = Book::where('item_category','自分自身の悩み')
        ->orderBy('created_at', 'asc')
        ->paginate(3);
        return view('books', [
            'books' => $company
